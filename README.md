...
[![Actions Status](https://github.com/antonpirker/Publish-Docker-Github-Action/workflows/Test/badge.svg)](https://github.com/antonpirker/Publish-Docker-Github-Action/actions)
[![Actions Status](https://github.com/antonpirker/Publish-Docker-Github-Action/workflows/Integration%20Test/badge.svg)](https://github.com/antonpirker/Publish-Docker-Github-Action/actions)
[![Actions Status](https://github.com/antonpirker/Publish-Docker-Github-Action/workflows/Integration%20Test%20Github/badge.svg)](https://github.com/antonpirker/Publish-Docker-Github-Action/actions)
...
**Changes from the original version:** Adds flags `disable_build` and `disable_push` that can be used to preventing building or pushing of the container. (For building the container in one step, then for example running tests in the container, and then in a separate step pushing the aldready built container.)
...
| Registry                                             | Action                                        |
|------------------------------------------------------|-----------------------------------------------|
| Amazon Webservices Elastic Container Registry (ECR)  | https://github.com/antonpirker/ecr-login-action    |
| Google Cloud Container Registry                      | https://github.com/antonpirker/gcloud-login-action |

## Outputs

`tag` is the tag, which was pushed  
`snapshot-tag` is the tag that is generated by the [snapshot-option](https://github.com/antonpirker/Publish-Docker-Github-Action#snapshot) and pushed
...
### disable_build
Set `disable_build` to true to prevent the `docker build` step to be performed.
...
### disable_push
Set `disable_push` to true to prevent the `docker push` step to be performed.
```yaml
with:
  name: myDocker/repository
  username: ${{ secrets.DOCKER_USERNAME }}
